...
build_kernel_module() {
    ...
    log "Adding the Odin 3 sns_iio patch"

    install -m 0644 \
        "$patch_source" \
        "$kernel_dir/patches/$SNS_PATCH_NAME"

    grep -qxF "$SNS_PATCH_NAME" "$kernel_dir/patches/series" ||
        printf '\n%s\n' "$SNS_PATCH_NAME" >>"$kernel_dir/patches/series"

    grep -qxF 'CONFIG_SNS_IIO=m' \
        "$kernel_dir/config/armada-kernel.config.overrides" ||
        printf '\n# AYN Odin 3 Snapdragon Sensor Core IMU bridge\nCONFIG_SNS_IIO=m\n' \
            >>"$kernel_dir/config/armada-kernel.config.overrides"
...
if grep -q '^CONFIG_MODVERSIONS=y' .config; then
    echo "==> CONFIG_MODVERSIONS=y: building all modules to generate exact symbol CRCs"
...
test -f drivers/iio/imu/sns_iio.ko

install -Dm644 \
    drivers/iio/imu/sns_iio.ko \
    "${OUT_DIR}/sns_iio.ko"
...
CONTAINER
    ...
    log "Building sns_iio.ko for kernel $(uname -r)"
    ...
    sudo chown -R "$(id -u):$(id -g)" "$WORK_ROOT/output"
